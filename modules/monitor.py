...
def render_large_layout(state, width=480, height=320):
    """
    Renders honeypot state to a PIL Image for large color TFT displays.
    Landscape layout (480x320) with two columns:
      Left:  LED status circles + attack count
      Right: Attack info (top) + system health (bottom)

    Designed for ILI9486-based 3.5" SPI displays (MHS-3.5inch)
    landscape 480x320 with rotate=90 driver setting.
    """
    try:
        from PIL import Image, ImageDraw, ImageFont
    except ImportError:
    ...
    try:
        ...
        font_hdr = ImageFont.truetype(FONT_BOLD, 13)
        font_lbl = ImageFont.truetype(FONT_PATH, 11)
        font_big = ImageFont.truetype(FONT_BOLD, 28)
    except Exception:
        font_sm = font_md = font_hdr = font_lbl = font_big = ImageFont.load_default()

    now_str = datetime.now(timezone.utc).strftime("%H:%M UTC")

    # Title bar
    TITLE_H = 22
    draw.rectangle([(0, 0), (width, TITLE_H)], fill=(25, 25, 55))
    draw.text((6, 4),   "Honeypot Kit", font=font_hdr, fill=CYAN)
    draw.text((145, 5), f"IP: {state.ip_address}", font=font_sm, fill=WHITE)
    draw.text((width - 100, 5), f"up {state.uptime_str}  {now_str}", font=font_sm, fill=GREY)

    # Layout geometry
    COL_LEFT_W  = 185
    COL_RIGHT_X = COL_LEFT_W + 4
    CONTENT_Y   = TITLE_H + 4
    CONTENT_H   = height - TITLE_H - 22
    FOOTER_Y    = height - 20
    MID_Y       = CONTENT_Y + CONTENT_H // 2

    # Left column panel
    draw.rectangle(
        [(2, CONTENT_Y), (COL_LEFT_W, CONTENT_Y + CONTENT_H)],
        fill=PANEL_BG, outline=PANEL_EDGE
    )
    draw.text((6, CONTENT_Y + 4), "STATUS", font=font_hdr, fill=GREY)
    ...
    cx2 = COL_LEFT_W // 2
    cx3 = (COL_LEFT_W * 3) // 4
    _draw_led(draw, cx1, led_y, led_r, GREEN_ON,  GREEN_OFF,  g_on, "GRN", font_lbl)
    _draw_led(draw, cx2, led_y, led_r, YELLOW_ON, YELLOW_OFF, y_on, "YLW", font_lbl)
    _draw_led(draw, cx3, led_y, led_r, RED_ON,    RED_OFF,    r_on, "RED", font_lbl)

    # State label centered
    state_labels = {
        "healthy":        ("Healthy",       WHITE),
        "login_history":  ("Login Alert!",  RED_ON),
        "active_session": ("Active Session",YELLOW_ON),
        "high_rate":      ("High Rate!",    RED_ON),
        "warning":        ("Warning",       YELLOW_ON),
        "cowrie_down":    ("Cowrie Down!",  RED_ON),
        "critical":       ("CRITICAL",      RED_ON),
    }
    state_text, state_color = state_labels.get(led_state, (led_state, WHITE))
    try:
        bbox = draw.textbbox((0, 0), state_text, font=font_sm)
        tw = bbox[2] - bbox[0]
    except Exception:
        tw = len(state_text) * 7
    draw.text((max(4, (COL_LEFT_W - tw) // 2), led_y + led_r + 18),
              state_text, font=font_sm, fill=state_color)

    # Attack count large number
    atk_color = RED_ON if state.attack_count > 100 else AMBER if state.attack_count > 10 else WHITE
    draw.text((8,  CONTENT_Y + 130), "ATTACKS", font=font_lbl, fill=GREY)
    draw.text((8,  CONTENT_Y + 144), str(state.attack_count), font=font_big, fill=atk_color)
    rate_color = RED_ON if state.attack_rate >= 10 else AMBER if state.attack_rate > 0 else GREEN_ON
    draw.text((8,  CONTENT_Y + 178), f"Rate: {state.attack_rate}/min", font=font_sm, fill=rate_color)
    sess_color = AMBER if state.active_sessions > 0 else GREY
    draw.text((8,  CONTENT_Y + 196), f"Sessions: {state.active_sessions}", font=font_sm, fill=sess_color)

    # Top right - attack info
    draw.rectangle(
        [(COL_RIGHT_X, CONTENT_Y), (width - 2, MID_Y - 2)],
        fill=PANEL_BG, outline=PANEL_EDGE
    )
    draw.text((COL_RIGHT_X + 6, CONTENT_Y + 4), "ATTACK INFO", font=font_hdr, fill=GREY)
    last_ip = state.last_attacker if state.last_attacker and state.last_attacker != "none" else "none yet"
    draw.text((COL_RIGHT_X + 6, CONTENT_Y + 22), "Last attacker:", font=font_sm, fill=GREY)
    draw.text((COL_RIGHT_X + 6, CONTENT_Y + 38), last_ip[:26], font=font_sm, fill=WHITE)

    # Bottom right - system health
    draw.rectangle(
        [(COL_RIGHT_X, MID_Y + 2), (width - 2, CONTENT_Y + CONTENT_H)],
        fill=PANEL_BG, outline=PANEL_EDGE
    )
    draw.text((COL_RIGHT_X + 6, MID_Y + 4), "SYSTEM", font=font_hdr, fill=GREY)
    ...
    RX = COL_RIGHT_X + 6
    draw.text((RX, MID_Y + 20), f"Disk:   {state.disk_pct}%", font=font_sm, fill=disk_color)
    draw.text((RX, MID_Y + 36), f"Mem:    {state.mem_pct}%",  font=font_sm, fill=mem_color)
    draw.text((RX, MID_Y + 52), f"Cowrie: {cowrie_text}",     font=font_sm, fill=cowrie_color)

    bar_x = COL_RIGHT_X + 110
    bar_w = width - bar_x - 10
    for bar_y_off, pct, color in [(24, state.disk_pct, disk_color), (40, state.mem_pct, mem_color)]:
        by = MID_Y + bar_y_off
        draw.rectangle([(bar_x, by), (bar_x + bar_w, by + 6)], fill=(40, 40, 60))
        fw = int(bar_w * pct / 100)
        if fw > 0:
            draw.rectangle([(bar_x, by), (bar_x + fw, by + 6)], fill=color)

    # Footer
    draw.rectangle([(0, FOOTER_Y), (width, height)], fill=(25, 25, 55))
    draw.text((6, FOOTER_Y + 4), "github.com/ericburnsonline/honeypot-kit", font=font_lbl, fill=(60, 60, 90))
